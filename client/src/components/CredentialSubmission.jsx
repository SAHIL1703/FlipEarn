import { CirclePlus, CirclePlusIcon, X } from "lucide-react";
...
const CredentialSubmission = ({ onClose, listing }) => {
  ...
  const handleAddFeild = () => {
    ...
    if (!name) {
      return toast("Please Enter A Field Name");
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4">
      <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-[320px] flex flex-col">
        {/* Header  */}
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="ml-4 p-1 hover:bg-white/20 hover:bg-opacity-20 rounded-lg transition-color"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="outline-none border-b border-gray-200 w-full"
            />
            <button
              type="button"
